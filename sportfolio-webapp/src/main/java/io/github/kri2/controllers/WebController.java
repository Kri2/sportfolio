...
public class WebController {
	...
	public String serveWelcome(Model model){
        ...
        */
		...
		String string = restTemplate.getForObject("http://finance.google.com/finance/info?client=ig&q=AAPL", String.class);
        string=string.replace("//","").replace("[","").replace("]","").replace("\n","").replace("\t","");
        System.out.println(string);//pytanie jak to ładnie przyporządkować do obiektu Stock teraz?
		...
		try {
			...
			System.out.println(stock1.getL_cur());
		} catch (IOException e) {
